...
export const createUsuario = async (req, res) => {
  const { email, password, username , estado} = req.body;
  console.log(email)
  ...
  try {
    ...
    res.status(500).json({ message: "se cayo" });
  }
};
...
export const updateUsuario = async (req, res) => {
  const { id } = req.params;
  const { username, email, estado } = req.body; // Agregar roles si lo deseas
  try {
    const updatedUser = await User.findByIdAndUpdate(id, { username, email, estado }, { new: true });
